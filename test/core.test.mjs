...
t("drawing: the sheet's own measurements reach the model, not just a count of them", () => {
  /* THE IMPORTER READS OVER A THOUSAND DIMENSIONS ACROSS DYLAN'S SET, matched to real line work
     by arithmetic at 0.00% error, plus ten survey points to four decimals. Until 2026-09-21
     every one of them was parsed, COUNTED, shown as a number in the picker and then dropped:
     `IMP.cur.drawing` carried the plot scale and the crop and nothing else.

     That is `manual input for PICTURES, never for information` broken at the last step. Reading
     a dimension is worth nothing if the person still has to type it.

     `drawingStated` returns a RANGE and a list, never one number. Nothing here knows which of
     eleven dimensions corresponds to the span somebody traced, and picking the largest would be
     precisely the plausible-and-wrong answer this project keeps having to retract. */
  const A = API;
  const dims = [
    {text: `4'-0"`,  mm: 1219.2, error: 0.0,  detail: "MAIN COLUMN FRONT ELEVATION"},
    {text: `12'-4"`, mm: 3759.2, error: 0.0,  detail: "MAIN COLUMN FRONT ELEVATION"},
    {text: `2'-10"`, mm: 863.6,  error: 0.08, detail: "MAIN COLUMN FRONT ELEVATION"},
  ];
  const got = A.drawingStated({dims, dimsAreThisDetail: true});
  ok(got, "a drawing carrying dimensions must report them");
  eq(got.n, 3);
  near(got.min, 863.6, 0.01, "the smallest stated dimension");
  near(got.max, 3759.2, 0.01, "and the largest — 12'-4\" is 3759.2mm, not a rounded 3759");
  eq(got.forThisDetail, true, "it has to say whether these belong to the detail or the page");
  ok(got.texts.includes(`4'-0"`), "carry the text AS WRITTEN — 4'-0\" is what the sheet says");

  eq(A.drawingStated(null), null);
  eq(A.drawingStated({}), null, "a drawing with no dimensions has nothing to state");
  eq(A.drawingStated({dims: []}), null);
  eq(A.drawingStated({dims: [{text: "SCALE:", mm: null}]}), null,
     "an entry with no measurement is not a measurement — junk must not become a range");
  eq(A.drawingStated({dims: [{text: "x", mm: 0}]}), null, "nor a zero");

  /* and the record the tracer receives must actually CARRY them. Comments stripped first: this
     file has been fooled three times by prose naming the thing it was checking for. */
  const src = html.replace(/\/\*[\s\S]*?\*\//g, " ").replace(/\/\/[^\n]*/g, " ");
  const i = src.indexOf("IMP.cur.drawing =");
  ok(i > 0, "the drawing record must still be findable");
  const rec = src.slice(i, i + 500);
  ok(/dims\s*:/.test(rec), "the dimensions the importer read have to travel with the drawing");
  ok(/points\s*:/.test(rec),
     "and the survey schedule — that is placement data, and Workshop is where it is going");
});
